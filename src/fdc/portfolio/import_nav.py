...
def import_nav_csv(
    ...
    _log_issues(session, batch.id, validation.issues)
...
def _stage_source_artifact(nav_csv_path: Path, batch_key: str, artifacts_dir: Path | None = None) -> Path:
    base_artifacts = artifacts_dir or nav_csv_path.parents[2] / "data" / "artifacts"
...
def _log_issues(session: Session, batch_id: int, issues: list[ValidationIssue]) -> None:
    if not issues:
        return
